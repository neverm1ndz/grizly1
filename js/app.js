(() => {
    ...
    (() => {
        ...
        $(".form").on("submit", (function(event) {
            ...
            let form = this, data = ($(".submit", form), new FormData), files = $("input[type=file]"), nameInput = $('[name="name"]', form), name = nameInput.val(), emailInput = $('[name="email"]', form), email = emailInput.val(), emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/, phoneInput = $('[name="phone"]', form), phone = iti.getNumber(), isValidPhone = iti.isValidNumber(), checkboxInput = $('[name="agreement"]', form), checkbox = checkboxInput.prop("checked");
            if ("" === name.trim()) nameInput.addClass("_form-error"); else nameInput.removeClass("_form-error");
            if (!emailRegex.test(email)) emailInput.addClass("_form-error"); else emailInput.removeClass("_form-error");
            if (!isValidPhone) phoneInput.addClass("_form-error"); else phoneInput.removeClass("_form-error");
            if (!checkbox) checkboxInput.addClass("_form-error"); else checkboxInput.removeClass("_form-error");
            if ("" === name.trim() || !emailRegex.test(email) || !isValidPhone) {
                tostConfig.text = "Пожалуйста, заполните корректно поля";
                tostConfig.style.background = "linear-gradient(to right, #ca3433, #fa8072)";
                Toastify(tostConfig).showToast();
                return false;
            } else if (!checkbox) {
                tostConfig.text = "Необходимо принять условия обработки персональных данных.";
                tostConfig.style.background = "linear-gradient(to right, #ca3433, #fa8072)";
                Toastify(tostConfig).showToast();
                return false;
            }
